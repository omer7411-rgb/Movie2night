...
async def get_jaffa_data_with_screenshot():
    results = []
    screenshot_path = "jaffa_debug.png"
    async with async_playwright() as p:
        ...
        try:
            # ניסיון גישה ישיר לעמוד הלו"ז
            url = "https://www.jaffacinema.com/schedule"
            await page.goto(url, wait_until="networkidle", timeout=60000)
            
            # המתנה משמעותית לטעינת האלמנטים של Wix
            await page.wait_for_timeout(8000)
            
            # צילום מסך לדיבגינג - נשמר בשרת ויוצג למשתמש
...
if st.button("🔄 טען לוח וצלם מסך", type="primary"):
    with st.spinner("סורק את האתר ומצלם..."):
        movies, img_file = asyncio.run(get_jaffa_data_with_screenshot())
        
        # הצגת צילום המסך לדיבגינג
        if os.path.exists(img_file):
            st.subheader("📸 מה הסורק רואה כרגע:")
            st.image(img_file, caption="צילום מסך בז
