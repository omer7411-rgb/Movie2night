...
st.set_page_config(page_title="קולנוע יפו - לוח נקי", page_icon="🎬", layout="wide")
...
st.markdown("""
    ...
    .movie-meta { color: #8b949e; font-size: 1.1rem; direction: rtl; }
    .buy-btn {
...
async def scrape_clean(status_placeholder):
    results = []
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for i in range(12):
                status_placeholder.info(f"טוען סרטים... ({i+1}/12)")
...
if st.session_state.movies is None:
    if st.button("🔍 טען את לוח ההקרנות", type="primary"):
        msg = st.empty()
        st.session_state.movies = asyncio.run(scrape_clean(msg))
        msg.empty()
        st.rerun()
else:
    # הגדרות זמן וסינון
    ...
    with st.sidebar:
        ...
        st.download_button("📥 הורד ליומן גוגל", ical_data, "cinema.ics")
